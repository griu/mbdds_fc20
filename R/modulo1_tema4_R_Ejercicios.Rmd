...
output:
  ...
  html_document:
    df_print: paged
subtitle: 'TEMA 4.1. HERRAMIENTAS DE ANALISIS: PROGRAMACIÓN EN R - MICROACTIVIDADES'
---
...
Para el ejercicio 1, utilizaremos los datos `millas` que hay en el package `datos`. Estos datos consisten en 238 y 11 variables que describen el consumo de combustible de 38 modelos de coche populares.
...
Crea una función que tome un vector como input y retorne una lista con los siguientes resultados:
...
Escribe un bucle for que calcule la media todas las columnas numéricas de `millas`. 
...
¿Qué parámetros son necesarios para especificar la lectura de datos de ancho fijo mediante: `read.fwf()`?
...
> Utiliza la función `merge()` para juntar la table de 2.5 con `millas`.
...
Utiliza las funciones del package dplyr: `group_by()` y `mutate()` para realizar el mismo cálculo que en 2.5. y 2.6. sin necesidad de utilizar `merge()`. Llama a la nueva columna "autopista_clase_dplyr"
...
Explica por qué el resultado de fechahora_1 y fechahora_2 son distintos en la siguiente expresión:
