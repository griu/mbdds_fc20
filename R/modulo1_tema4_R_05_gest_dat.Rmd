...
Para definir las claves del cruce, es necesario pasar como parámetro los nombre de las columnas:
...
El siguiente ejemplo, realiza un *FULL JOIN* definiendo como clave la columna `ID`.
...
El parámetro `na.rm=TRUE` evita que las funciones resumen, devuelvan un NA debido a la presencia de NA s en la columna `pib_por_capita`. Observa como esto sucede en la siguiente función. 
