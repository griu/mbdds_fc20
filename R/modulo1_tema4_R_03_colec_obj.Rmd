...
Se puede generalizar las matrices a mas de 2 dimensiones con `array()`.
...
Se utiizan las mismas reglas que en las matrices para saber la dimensión, nombres de filas y columnas.
...
En listas y data frames se puede obtener un elemento con `$nombre` , o bién, con corchetes dobles
...
Se seleccionan las posiciones con valores `TRUE` o `T` y se decartan las posiciones `FALSE` o `F`.
...
El operador `%in%` tiene un funcionamiento un poco distinto. Evalua cada elemento del objeto situado a su izquierda respecto a todos los  elementos del objeto situado a su derecha.
...
Para seleccionar los registos con missing en una columna.
...
La pesencia de NA en una variable dificulta la definición de condiciones lógicas.  Por ejemplo, si seleccionamos países con PIB per cápita inferior a 500 dólares se obtiene el siguiente resultado incorrecto:
...
> \<recuerda\>Utiliza which para "protejerte" de la presencia de valores faltantes en la definición de filtros.\</recuerda\>
